...
func UpadateBookCopies(c *gin.Context) {
	...
	if userData.Role == "admin" {
		...
		if checkBook.TotalCopies < 0 {
			c.JSON(http.StatusBadGateway, gin.H{"message": "Decrease count is more than total copies"})
			return
		}
		if checkBook.AvailableCopies < 0 {
			c.JSON(http.StatusBadGateway, gin.H{"message": "Decrease count is more than available copies"})
			return
		}
		if checkBook.TotalCopies == 0 {
